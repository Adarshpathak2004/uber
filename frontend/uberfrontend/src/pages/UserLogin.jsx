      ...
      <div>
        ...
        <form onSubmit={submitHandler} noValidate>
          ...
          <input
            ...
            className={`bg-[#eeeeee] rounded-lg px-4 py-2 border w-full text-lg placeholder:text-base outline-none focus:ring-2 transition ${email && !isValidEmail ? 'ring-2 ring-red-400 border-red-400' : 'focus:ring-black/30'} mb-1`}
            type="email"
            placeholder='email@example.com'
            aria-invalid={email && !isValidEmail}
          />
          {email && !isValidEmail && (
            <p className='text-sm text-red-500 mb-5'>{errors.email || 'Invalid email format'}</p>
          )}
          ...
          {password && password.length < 6 && (
            <p className='text-sm text-red-500 mt-1 mb-4'>{errors.password || 'Use 6+ characters'}</p>
          )}
